...
class Shrink{
	...
	*/
	function build($files, $type, $cacheFile='') {
		...
		$cacheFilePath 	  = preg_replace('/(\/+|\\+)/', DS, WWW_ROOT . $this->settings[$type]['cachePath'] . $cacheFile);
		$webCacheFilePath = $this->settings['url'] .SUBDIR_CSS_PATH. preg_replace('/\/+/', '/', '/'. $this->extraPath .'/'. $this->settings[$type]['cachePath'] . $cacheFile);
