...
<div class="mainbox">
  <div class="inerbigbox">
	...
	<div  class="top-textbox" >
		...
		<input type="hidden" name="uuid" value="<?php echo $uuid = isset($data['uuid']) ? $data['uuid'] : md5(uniqid(rand(), true));  ?>">
	</div>
		...
		<ul>
		   ...
		   if(!empty($data['trip'])){
			   foreach($data['trip'] as $v){ 
				   echo $this->element("next_level",['uniqueId' => $v['unique_id'].rand(5000,99999),'edit' => $v,'time' => $timeKey]);
	...
	<?php } ?>
	 <div class="nexsteptrip nexsteptrip1"><a class="tripviewmy" onclick="$('#tripviewmy<?php echo $timeKey; ?>').toggle();planSave<?php echo $timeKey; ?>();" href="javascript:void(0);">Show / hide full itinerary</a></div>
  </div>
</div>

<div id="tripviewmy<?php echo $timeKey; ?>" style="<?php if($this->request->params['action'] != 'tripView'){ ?>display:none"<?php } ?>>
	<div class="mainbox">
...
<?php if(!isset($data)){ ?>
	...
	$.get("<?php echo $this->Url->build(['plugin' => '','controller' => 'users','action' => 'next_level']); ?>?time=<?php echo $timeKey; ?>", function(data, status){
...
<?php }else{ ?>
	...
	setTimeout(function(){
		...
		var theTemplate		  =  Handlebars.compile(theTemplateScript);
		var context			  =  {"text":context.name,"url":context.slug,"pdf":context.pdf};
		var theCompiledHtml   =  theTemplate(context);	
		$('.social_url').html(theCompiledHtml);
	},1000);
...
$(document).on('click', '.next_level<?php echo $timeKey; ?>', function(){
	...
	$.get("<?php echo $this->Url->build(['plugin' => '','controller' => 'users','action' => 'next_level']); ?>?date="+date111+"&time=<?php echo $timeKey; ?>", function(data, status){
        $("#ul-append<?php echo $timeKey; ?>").append(data);
...
<?php if(!$this->request->session()->read('Auth.User')){ ?>
	$(".url-editable").html('<a href="<?php echo WEBSITE_URL.'trip_edit/'.$uuid; ?>"><?php echo WEBSITE_URL.'trip_edit/'.$uuid; ?></a>');	
<?php }
if(isset($getmethod)){
	$this->Html->scriptEnd();		
}else{
